...
/execute as @e[tag=zoology_crafter] at @s if block ~ ~ ~ barrel run replaceitem block ~ ~ ~ container.0 beacon{CustomModelData:7}
/execute as @e[tag=zoology_crafter] at @s if block ~ ~ ~ barrel run replaceitem block ~ ~ ~ container.18 beacon{CustomModelData:8}
