...
#Egg
execute as @s[tag=pachycephalosaurus_egg,tag=hatch] run summon drowned ~ ~ ~ {NoGravity:1b,Silent:1b,PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b,Health:10f,Tags:["pachycephalosaurus","dinosaur","prepare"],Attributes:[{Name:generic.maxHealth,Base:10}]}
execute as @s[tag=pachycephalosaurus_egg,tag=hatch] unless entity @e[tag=pachycephalosaurus_grounds,distance=..200] run summon armor_stand ~ ~ ~ {NoGravity:1b,Invulnerable:1b,Small:1b,Marker:1b,Invisible:1b,PersistenceRequired:1b,Tags:["pachycephalosaurus_grounds","grounds","herbivorous_grounds"]}
...
execute as @s[tag=pachycephalosaurus,tag=prepare] run scoreboard players set @s Current_Food 2000
execute as @s[tag=pachycephalosaurus,tag=prepare] run tag @s remove prepare
#adult
