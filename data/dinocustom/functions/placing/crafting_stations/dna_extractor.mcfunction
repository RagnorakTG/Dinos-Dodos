...
execute as @s at @s unless entity @e[type=!firework_rocket,type=!item,distance=..0.1] align xyz if block ~ ~ ~ air if block ~1 ~ ~ chest positioned ~.5 ~ ~.5 run setblock ~ ~ ~ minecraft:chest[type=left,facing=south]{CustomName:"{\"text\":\"dna_extractor\"}"}
execute as @s[y_rotation=135.1..180] at @s unless entity @e[type=!firework_rocket,type=!item,distance=..0.1] align xyz if block ~ ~ ~ chest[type=left]{CustomName:"{\"text\":\"dna_extractor\"}"} positioned ~.5 ~ ~.5 run summon armor_stand ~ ~ ~ {NoGravity:1b,Small:1b,Invulnerable:1b,Invisible:1b,NoBasePlate:0b,Tags:["CustomBlock3"],DisabledSlots:4144896,ArmorItems:[{},{},{},{id:"minecraft:firework_rocket",Count:1b,tag:{CustomModelData:7}}]}
...
kill @s
tag @s[tag=CustomBlock3] add crafting_station
