...
#moar
execute as @s at @s unless block ~ ~ ~ air run summon item ~ ~.6 ~ {Item:{id:"minecraft:firework_rocket",Count:1b,tag:{display:{Name:"{\"text\":\"Genome Modifier\",\"italic\":false}"},CustomModelData:5,CustomBlock5:1b,Fireworks:{}}}}
...
execute as @s at @s unless entity @e[type=!firework_rocket,type=!item,distance=..0.1] align xyz if block ~ ~ ~ air unless block ~1 ~ ~ chest positioned ~.5 ~ ~.5 run setblock ~ ~ ~ minecraft:chest[type=left]{CustomName:"{\"text\":\"genome_modifier\"}"}
execute as @s at @s unless entity @e[type=!firework_rocket,type=!item,distance=..0.1] align xyz if block ~ ~ ~ air if block ~1 ~ ~ chest positioned ~.5 ~ ~.5 run setblock ~ ~ ~ minecraft:chest[type=left,facing=south]{CustomName:"{\"text\":\"genome_modifier\"}"}
execute as @s[y_rotation=135.1..180] at @s unless entity @e[type=!firework_rocket,type=!item,distance=..0.1] align xyz if block ~ ~ ~ chest[type=left]{CustomName:"{\"text\":\"genome_modifier\"}"} positioned ~.5 ~ ~.5 run summon armor_stand ~ ~ ~ {NoGravity:1b,Small:1b,Invulnerable:1b,Invisible:1b,NoBasePlate:0b,Tags:["CustomBlock5"],DisabledSlots:4144896,ArmorItems:[{},{},{},{id:"minecraft:firework_rocket",Count:1b,tag:{CustomModelData:5}}]}
...
#sound
execute as @s at @s run playsound minecraft:block.metal.place block @a[distance=..20] ~ ~ ~ 3 1
#killing the firework
kill @s
tag @e[tag=CustomBlock5] add crafting_station
