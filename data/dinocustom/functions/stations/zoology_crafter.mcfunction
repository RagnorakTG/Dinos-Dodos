...
# Replace Head
execute as @s[tag=zoology_crafter,tag=faced,tag=!replaced] at @s run replaceitem entity @s armor.head dispenser{CustomModelData:4040009}
...
kill @e[type=item,nbt={Item:{id:"minecraft:barrel",tag:{display:{Name:'{"text":"zoology_crafter"}'}}}}]
# crafting
function dinocustom:stations/crafting/zoology_crafter/main
...
## Fix barrel
execute as @s[tag=!placed] at @s run setblock ~ ~ ~ barrel{CustomName:"{\"text\":\"zoology_crafter\"}"} replace
execute as @s[tag=!placed] at @s run tag @s add placed
execute as @s[tag=zoology_crafter] at @s run data merge entity @s {Fire:100}
