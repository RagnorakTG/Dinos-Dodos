...
execute as @s[tag=!dnd.place] run setblock ~ ~ ~ dispenser[facing=up]{CustomName:'{"translate":"Zoology Crafter","italic":false}'}
execute as @s[tag=!dnd.place] run tag @s add dnd.place
# Replace Block
execute as @s[tag=!dnd.model] run replaceitem entity @s armor.head dispenser{CustomModelData:400001}
