...
## Yellow Name
execute if score temp2 dnd.dummy matches 0 run execute store result score temp dnd.dummy run data get block ~ ~ ~ Items[{Slot:5b}].tag.dnd.accuracy
execute if score temp2 dnd.dummy matches 0 run execute if score temp dnd.dummy matches 100.. run data modify block ~ ~ ~ Items[{Slot:5b}].tag.display.Name set value '{"italic":false,"color":"yellow","translate":"item.dnd.structural_note"}'
## Subtract from slots
